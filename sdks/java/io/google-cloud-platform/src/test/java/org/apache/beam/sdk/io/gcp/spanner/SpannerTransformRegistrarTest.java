...
public class SpannerTransformRegistrarTest  {
    ...
    public void testKnownBuilderInstances() {
        Map<String, ExternalTransformBuilder<?, ?, ?>> builderInstancesMap = spannerTransformRegistrar.knownBuilderInstances();
        ...
        assertThat(builderInstancesMap, IsMapContaining.hasKey(SpannerTransformRegistrar.INSERT_OR_UPDATE_URN));
    ...
    public void testReadBuilderBuildExternalWithTimestampBoundMode() throws InvalidProtocolBufferException {
        ...
        Stream.of(TimestampBound.Mode.values()).forEach( mode -> {
            ...
            configuration.setReadTimestamp(Timestamp.parseTimestamp("2077-10-15T00:00:00").toString());
            PTransform<PBegin, PCollection<Row>> readTransform = readBuilder.buildExternal(configuration);
            assertNotNull(readTransform);
        });
    }
